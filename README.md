...
Lifecycle node that records the path travelled by the robot. It collects `/loc/odom` poses, stores them as a 2D path with per-point speed, optionally smooths it, and saves it to a JSON file (`work_performed` Agri format) that can later be replayed by `nav_replay`.
...
3. **Record** — on each `/loc/odom` message, append the `(x, y)` position and store the rounded linear speed (minimum magnitude `0.1` m/s). The `working_zone` column is written for each point but is always `false` (no vehicle feedback is subscribed to update it).
...
- stores per-point speed, course, and a `working_zone` flag (always `false` from this node);
...
- per point: position, speed, curvature, and `working_zone` (always `false`)
